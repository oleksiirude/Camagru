<?php
	class componentMail
	{
		...
		{
			$login = $post['login'];
			$path = 'http://camagru.us-east-2.elasticbeanstalk.com/user/register/confirm/'.$token;
			$to = $post['email'];
		...
		{
			$login = $post['login'];
			$path = 'http://camagru.us-east-2.elasticbeanstalk.com/user/recover/password/confirm/'.$token;
			$to = $post['email'];
            $date = date('r', $_SERVER['REQUEST_TIME']);
			$headers = "From: camagrubot@gmail.com\r\n" .
				"Reply-To: no-reply\r\n" .
