let profile_uri = document.documentURI.substring(0, 38);
let tail_profile_uri = document.documentURI.substr(38) + '/';
profile_uri += tail_profile_uri.replace(/\/{2,}/, '/');

if (profile_uri === 'http://camagru.us-east-2.elasticbeanstalk.com/user/profile/') {
...
function addContent(parent, result) {
    ...
    for (let i = 0; i < result.length; i++) {
        ...
        if (document.documentURI === 'http://camagru.us-east-2.elasticbeanstalk.com/user/profile') {
            photo.addEventListener('click', removePostIntention);
            photo.style.cursor = 'pointer';
        }
        if (document.documentURI === 'http://camagru.us-east-2.elasticbeanstalk.com/') {
            let avatar, login;
